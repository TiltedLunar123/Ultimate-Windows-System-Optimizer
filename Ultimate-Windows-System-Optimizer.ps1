...
function Write-Good ([string]$Msg)    { Write-Host "    [OK]   $Msg" -ForegroundColor Green;    Log "[OK]   $Msg" }
function Write-Warn ([string]$Msg)    { Write-Host "    [WARN] $Msg" -ForegroundColor Yellow;   Log "[WARN] $Msg" }
function Write-Bad  ([string]$Msg)    { Write-Host "    [FAIL] $Msg" -ForegroundColor Red;      Log "[BAD]  $Msg" }
function Write-Fix  ([string]$Msg)    { Write-Host "    [FIX]  $Msg" -ForegroundColor Magenta;  Log "[FIX]  $Msg"; $script:TotalFixesApplied++ }
function Write-Skip ([string]$Msg)    { Write-Host "    [SKIP] $Msg" -ForegroundColor DarkGray; Log "[SKIP] $Msg" }
...
function Get-SystemAnalysis {
    ...
    $script:DeviceType  = if ($script:IsLaptop) { "Laptop" } else { "Desktop" }
...
function Show-FinalReport {
    ...
    Write-Host "    +-----------------------------------------------------+" -ForegroundColor Green
    Write-Host "    |                                                      |" -ForegroundColor Green
    Write-Host "    |  Device Type:    $($script:DeviceType.PadRight(34))|" -ForegroundColor Green
    Write-Host "    |  System Tier:    $("$($script:SystemTier)".PadRight(34))|" -ForegroundColor Green
    Write-Host "    |  Fixes Applied:  $("$($script:TotalFixesApplied) optimizations".PadRight(34))|" -ForegroundColor Green
    Write-Host "    |                                                      |" -ForegroundColor Green
...
function Start-Optimizer {
    ...
    # Admin check (belt-and-suspenders with #Requires -RunAsAdministrator)
